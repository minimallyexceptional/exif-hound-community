import ExifImage from '../entities/ExifImage';
...
export default class EXIFImageFactory {
    private image: ExifImage | null = null;
    private gpsFormatter: GPSFormatter;
    private dataFormatter: DataFormatter;
    private latitudeString: string | null;
    private longitudeString: string | null;
    ...
    createImage(ImageElement: HTMLImageElement, exifDataObject: any): ExifImage {
        ...
        // if (exifDataObject.thumbnail?.blob) {
        //     blobToDataURL(exifDataObject.thumbnail.blob).then((res) => {
        //         if (res) {
        //             this.image.ThumbnailData = res;
        //         }
        //     });
        // }
        ...
        // GPS
        this.image.GPSLatitude = this.parseLatitude(
            this.latitudeString,
            this.longitudeString
        ) || null;
        
        this.image.GPSLongitude = this.parseLongitude(
            this.latitudeString,
            this.longitudeString
        ) || null;
    ...
    private parseLatitudeString(exifDataObject: any): string | null {
    ...
    private parseLongitudeString(exifDataObject: any): string | null {
    ...
    private parseLatitude(lat: string | null, lon: string | null): number | null {
        if (lat && lon) {
    ...
    private parseLongitude(lat: string | null, lon: string | null): number | null {
        if (lat && lon) {
    ...
    // private parsePosition(lat: string | null, lon: string | null): { latitude: number | null; longitude: number | null } {
    //     if (lat && lon) {
    //         return this.gpsFormatter.formatPosition(lat, lon);
    //     } else {
    //         return {
    //             latitude: null,
    //             longitude: null,
